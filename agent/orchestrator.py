...
class AgentOrchestrator:
    def __init__(self,brain,runtime): self.brain=brain; self.runtime=runtime
    def handle(self,message,state)->OrchestrationResult:
        brain=self.brain.ask(message); executed=[]; blocked=[]
        for intent in brain.intents:
            try:
                request=type("Request",(),{"tool_name":intent.tool_name,"arguments":intent.arguments,"requested_by":"model"})()
            ...
            except Exception as exc: blocked.append({"tool":intent.tool_name,"success":False,"error":str(exc)})
