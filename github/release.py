...
class ReleaseWorkflow:
    def __init__(self, git, github, approval):
        self.git = git
        self.github = github
    ...
    def propose(self, branch, files, tests_passed, message):
        if not tests_passed:
            raise RuntimeError("Tests must pass before release proposal.")
        return ReleaseProposal(branch, message, list(files), True, True)

    def approve_and_execute(self, proposal):
        if not self.approval.approve(proposal.branch):
            raise PermissionError("Release approval required.")
        self.git.commit(proposal.commit_message)
