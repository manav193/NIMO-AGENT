...
def test_tool_registry_registers_and_executes():
    registry = ToolRegistry()
    registry.register(ToolSpec("echo", "Echo input", RiskLevel.READ, ok_handler))
    result = registry.execute(ToolRequest("echo", {"value": "hello"}))
...
def test_runtime_records_input():
    audit = AuditLogger()
    ...
    assert audit.events[0]["action"] == "agent.input"
